...
export default function UserFormControlled({ onSubmitUser, defaultUser }: UserFormProps) {
    ...
    const onSubmit = (e: MouseEvent<HTMLButtonElement>) => { // MouseEvent<HTMLButtonElement> is the type of the event object and a typescript generic
        e.preventDefault();
        const isDelete = e.currentTarget.id === "delete-btn" ? true : undefined;
        onSubmitUser(user, isDelete); // onSubmitUser is a function from the parent component
        setUser({ ...emptyUser }); // a user object with empty values is unpacked into the setUser function that sets the user state to an empty user
    };
