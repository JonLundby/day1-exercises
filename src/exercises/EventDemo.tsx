...
export default function EventDemo1({ title }: BaseProps) {
    ...
    return (
        <>
            <h2>{title}</h2>
            <div className="info">
                "Simple exercise to demonstrate how to pass event handlers to child components to let the parent react
                to the event. Here, we have a simple ImageButton where users (parents) can react on the click event" -
                LM
            </div>
            <ImageButton
